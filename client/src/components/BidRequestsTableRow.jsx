...
const BidRequestsTableRow = ({ bid, handleStatusChange }) => {
    const { _id, title, email, deadline, price, category, status } = bid || {};
    return (
        <tr>
            ...
            <td className='px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap'>
                <div
                    className={`inline-flex items-center px-3 py-1 rounded-full gap-x-2
                            ${status === 'Panding' && 'bg-yellow-100/60 text-yellow-500'}
                            ${status === 'In Proqress' && 'bg-blue-100/60 text-blue-500'}
                            ${status === 'Completed' && 'bg-green-100/60 text-green-500'}
                            ${status === 'Rejected' && 'bg-red-100/60 text-red-500'} `}
                >
                    <span
                        className={`h-1.5 w-1.5 rounded-full 
                            ${status === 'Panding' && 'bg-yellow-500'}
                            ${status === 'In Proqress' && 'bg-blue-500'}
            ...
            <td className='px-4 py-4 text-sm whitespace-nowrap'>
                <div className='flex items-center gap-x-6'>
                    <button
                        disabled={status === 'In Proqress' || status === 'Completed'}
                        onClick={() => handleStatusChange(_id, status, 'In Proqress')}
